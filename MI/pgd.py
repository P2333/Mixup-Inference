...
from .cleverhans_utils import clip_eta


def projected_gradient_descent(model_fn,
                               x,
                               eps,
                               eps_iter,
                               nb_iter,
  ...
  """
    ...
    if eps_iter < 0:
        raise ValueError(
            "eps_iter must be greater than or equal to 0, got {} instead".
            format(eps_iter))
    if eps_iter == 0:
        return x
    ...
    i = 0
    while i < nb_iter:
        adv_x = fast_gradient_method(model_fn,
                                     adv_x,
                                     eps_iter,
                                     norm,
                                     clip_min=clip_min,
